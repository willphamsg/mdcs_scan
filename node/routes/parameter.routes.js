...
const dataPath = path.join(__dirname, '../data/parameter.json');
...
function applyFilters(items, filter) {
  ...
  return items.filter(item => {
    ...
    return matchDepot && trialDateMatch && matchStatus;
  });
}
...
// POST get items
router.post('/trial/search', async (req, res) => {
  try {
    const db = await readData();
    const items = db['trial_device_list'];
    let data = [...items];
    ...
    const result = {
      ...
      timestamp: Date.now(),
      message: 'Trial device list with pagination details',
      payload: {
