...
class Locale {
    ...
    static create(locale, numberingSystem, outputCalendar, weekSettings, defaultToEN = !1) {
    ...
    isEnglish() {
        ...
        !!~["en", "en-us"].indexOf(this.locale.toLowerCase()) ||
            new Intl$1.DateTimeFormat(this._intl).resolvedOptions().locale.startsWith("en-us"));
    }
    listFormatter(opts = {}) {
    ...
    _supportsFastNumbers() {
        if (this.numberingSystem && this.numberingSystem !== "latn") {
            ...
            return (this.numberingSystem === "latn" ||
                ...
                Intl$1.DateTimeFormat(this._intl).resolvedOptions().numberingSystem === "latn");
        }
    }
}
...
let now = () => Date.now(), defaultZone = "system", defaultLocale, defaultNumberingSystem, defaultOutputCalendar, twoDigitCutoffYear = 60, throwOnInvalid = !1, defaultWeekSettings;
/**
...
function usesLocalWeekValues(obj, loc) {
    const hasLocaleWeekData = isDefined(obj.localWeekday) ||
...
function hasLocaleWeekInfo() {
    try {
        return (typeof Intl$1 !== "undefined" &&
            !!Intl$1.Locale &&
            ("weekInfo" in Intl$1.Locale.prototype || "getWeekInfo" in Intl$1.Locale.prototype));
    }
    catch (e) {
        return false;
    }
...
function parseZoneInfo(ts, offsetFormat, locale, timeZone) {
    ...
    const parsed = new Intl$1.DateTimeFormat(locale, modified)
        .formatToParts(date)
...
function extractISODuration(match) {
    ...
    const maybeNegate = (num, force = !1) => typeof num === typeof 0 && (force || (num && hasNegativePrefix)) ? -num : num;
    return [{
...
class Duration {
    ...
    shiftTo(...units) {
        ...
        Object.keys(accumulated).forEach((key) => {
            ...
            if (v !== 0) {
                built[lastUnit] +=
                    key === lastUnit ? v : v / this._matrix[lastUnit][key];
            }
        });
    ...
    toHuman(opts = {}) {
        ...
        const maxUnit = this.getMaxUnit(!0);
...
class TokenParser {
    ...
    explainFromTokens(input) {
        if (!this.isValid) {
            ...
            const [rawMatches, matches] = match(input, this.regex, this.handlers), [result, zone, specificOffset] = matches
...
class DateTime {
    ...
    static fromFormatParser(text, formatParser, opts = {}) {
        ...
        if (!localeToUse.equals(formatParser.locale)) {
            throw new InvalidArgumentError(`fromFormatParser called with a locale of ${localeToUse}, ` +
    ...
    static fromObject(obj = {}, opts = {}) {
        ...
        const tsNow = Settings.now(), offsetProvis = isNumber(opts.specificOffset)
            ? opts.specificOffset
            : zoneToUse.offset(tsNow), containsOrdinal = isDefined(normalized.ordinal), containsGregorYear = isDefined(normalized.year), containsGregorMD = isDefined(normalized.month) || isDefined(normalized.day), containsGregor = containsGregorYear || containsGregorMD, definiteWeekDef = normalized.weekYear || normalized.weekNumber;
        // cases:
         ...
         **/
        if (useWeekData) {
            return DateTime._buildObject(config, orderedWeekUnits, defaultWeekUnitValues, gregorianToWeek(tmpNow, minDaysInFirstWeek, startOfWeek));
        }
    ...
    static _buildObject(config, units, defaultValues, objNow) {
        ...
        const higherOrderInvalid = config.useWeekData
            ? hasInvalidWeekData(config.normalized)
            : config.containsOrdinal
                ? hasInvalidOrdinalData(config.normalized)
                : hasInvalidGregorianData(config.normalized);
        const invalid = higherOrderInvalid || hasInvalidTimeData(config.normalized);
        if (invalid) {
            return DateTime.invalid(invalid);
        }
        // compute the actual time
        const gregorian = config.useWeekData
            ? weekToGregorian(config.normalized)
            : config.containsOrdinal
                ? ordinalToGregorian(config.normalized)
                : config.normalized, [tsFinal, offsetFinal] = objToTS(gregorian, config.offsetProvis, config.zoneToUse), inst = new DateTime({
            ts: tsFinal,
    ...
    diff(otherDateTime, unit = "milliseconds", opts = {}) {
        if (!this.isValid || !otherDateTime.isValid) {
            const reason = (this.invalidReason || otherDateTime.invalidReason); // One of the two is certainly a string
    ...
    toRelative(options = {}) {
        ...
        return DateTime._diffRelative(base, this.plus(padding), Object.assign(Object.assign({}, options), { numeric: "always", units,
            unit }));
    }
    /**
    ...
    toRelativeCalendar(options = {}) {
        ...
        return DateTime._diffRelative(options.base || DateTime.fromObject({}, { zone: this.zone }), this, Object.assign(Object.assign({}, options), { numeric: "auto", units: ["years", "months", "days"], calendary: true }));
    }
    /**
     ...
     * @return {string}
     */
    toSQLTime({ includeOffset = !0, includeZone = !1, includeOffsetSpace = !0 } = {}) {
     ...
     */
    _adjustTime(dur) {
        const previousOffset = this._o, year = this._c.year + Math.trunc(dur.years), month = this._c.month + Math.trunc(dur.months) + Math.trunc(dur.quarters) * 3, c = Object.assign(Object.assign({}, this._c), { year,
            month, day: Math.min(this._c.day, daysInMonth(year, month)) +
                Math.trunc(dur.days) +
...
const VERSION = "4.6.2"; // REPLACED WITH ROLLUP

export { DateTime, Duration, FixedOffsetZone, IANAZone, Info, Interval, Intl$1 as Intl, InvalidZone, ORDERED_UNITS, REVERSE_ORDERED_UNITS, Settings, SystemZone, VERSION, Zone };
