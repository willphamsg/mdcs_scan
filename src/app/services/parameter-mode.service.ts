...
export class ParameterModeService {
  ...
  search(params: IParams): Observable<PayloadResponse> {
    if (environment?.useDummyData) {
      ...
      // return of(dummyData);
      console.log({ requestParameterModeParams: params });
      return this.http
  ...
  manage(params: IParameterMode[]): Observable<PayloadResponse> {
    return this.update(params);
  }

  add(params: IParameterMode[]): Observable<PayloadResponse> {
    if (environment?.useDummyData) {
      const dummyData: PayloadResponse = {
        status: 200,
        status_code: 'SUCCESS',
      ...
      // return of(dummyData);
      console.log('Adding Parameter Mode:', params);
      return this.http
