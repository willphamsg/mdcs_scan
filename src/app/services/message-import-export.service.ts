...
export class MessageImportExportService {
  ...
  sendExportRequest(count: number, date: string): Observable<PayloadResponse> {
    if (environment?.useDummyData) {
      const dummyData: PayloadResponse = {
        status: 200,
        status_code: 'SUCCESS',
      ...
      return this.http
        .post<PayloadResponse>(`${this.uri}export/send-file-request`, {
          count,
          date,
        })
        .pipe(
    ...
    return this.http
      .post<PayloadResponse>(`${this.uri}export/send-file-request`, { count })
      .pipe(
        catchError((err: HttpErrorResponse) => this.message.multiError(err))
      );
  }
