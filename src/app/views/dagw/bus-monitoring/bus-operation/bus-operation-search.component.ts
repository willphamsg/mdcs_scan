...
import { ManageBusOperationService } from '@services/bus-operation-status.service';
...
export class BusOperationSearchComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private busOperationService: ManageBusOperationService,
  ...
  loadFilterValues(): void {
    this.filterConfigs = [
      ...
      },
      {
        ...
        value: [],
        type: 'radio',
        options: this.connectStatusOptions?.map(item => ({
          value: item.id,
          label: item.value,
        })),
      },
