...
export class NewParameterApprovalSearchComponent implements OnInit, OnDestroy {
  ...
  subscribeToDepoChanges(): void {
    ...
    // });
    // this.depoService?.depoList$?.subscribe({
    //   next: (val: IDepoList[]) => {
    //     this.depots = val;
    //   },
    // });
  ...
  reloadHandler() {
    ...
    this.fetchNewParameterApprovalList();
    // }
  }

  fetchNewParameterApprovalList(): void {
    this.newParameterApprovalService.search(this.params).subscribe({
      next: (value: PayloadResponse) => {
        if (value.status == 200) {
          const source = value.payload['new_parameter_approval_list'];
          this.dataSource = source.map((item: INewParameterApproval) => {
            return <INewParameterApproval>{ ...item, chk: false };
          });
          this.selection = [];
          this.chkAll = false;
        }
  ...
  //             this.selection = response.payload['new_parameter_approval_list'];
