...
describe('BusTransferSearchComponent', () => {
  ...
  it('should check all', fakeAsync(() => {
    spyOn(component, 'checkAllHandler').and.callFake(() => null);

    fixture.detectChanges();
    const button = fixture.debugElement.query(
      val => val.attributes['id'] === 'chkAll'
    );
    expect(button).toBeTruthy();

    button.triggerEventHandler('change', {});
    tick(1000);
    expect(component.checkAllHandler).toHaveBeenCalled();
  }));
